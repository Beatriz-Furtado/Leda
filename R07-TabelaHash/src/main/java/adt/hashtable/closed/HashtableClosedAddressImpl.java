...
public class HashtableClosedAddressImpl<T> extends
	...
	public void insert(T element) {
		if (element != null && !this.isFull() && this.search(element) == null) {
			...
			if (this.table[indexHash] == null) {
				this.table[indexHash] = new LinkedList<T>();
			} else {
				this.COLLISIONS++;
			}
	...
	public void remove(T element) {
		if (element != null && !this.isEmpty()) {
			int indexHash = indexOf(element);
			
			if (indexHash != -1) {
				((LinkedList<T>) this.table[indexHash]).remove(element);
				this.elements--;
			}
		}
	}
	...
	public T search(T element) {
		...
		if (element != null && !this.isEmpty()) {
			...
			if (indexHash != -1) {
				LinkedList<T> list = (LinkedList<T>) this.table[indexHash];
				int indexList = list.indexOf(element);
				search = ((LinkedList<T>) this.table[indexHash]).get(indexList);
			}
		}
		return search;
	}
